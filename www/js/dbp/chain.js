...
dbp.chain.ChainService = {
    ...
    getLinks: function(context,model) { 
        var links = [];
      for (var i = 0; i < this.chainLinks.length; i++) {
          var currentLink = this.chainLinks[i];
          if (currentLink.match(context,model))
            links.push(currentLink);
      }
      return links;
    },
    trigger: function(context,model) {
        // Construct chain
        var links = this.getLinks(context,model);
...
dojo.declare("dbp.chain.Chain",null, { 
    ...
    next: function(newModel) { 
        if (newModel != undefined) {
               this.chainService.trigger(this.context, newModel);
               this.stop();
        } else {
            ...
            this.linkIndex++;
             var s = this;
            var result = this.currentLink.execute(this.context,this.model,function(m) { s.next(m); });
            if (result !== false) {
             if (result == undefined)
                s.next();
             else
                dojo.when(result,function(r) { s.next(r); });
            }
                }
        }
    }
